...
<div style="max-width: 900px; margin: 0 auto;" x-data="dumpPage()">
    ...
    @forelse($grouped as $date => $tasks)
        ...
        @php
            ...
            $wd = \App\Models\WorkingDay::where('day_number', $dateObj->dayOfWeek)->first();
        ...
        <div style="margin-bottom: 24px;">
            ...
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px; padding: 0 2px;">
                <span style="font-size: 15px; font-weight: 700; color: {{ $isToday ? '#7c3aed' : ($isPast ? '#dc2626' : '#1e293b') }};">{{ $dateObj->format('D, j M') }}{{ $isToday ? ' (Today)' : '' }}</span>
                @if($wd)
                ...
                @if($isPast && !$isToday)
                    <span style="padding: 2px 6px; border-radius: 4px; font-size: 9px; font-weight: 700; background: #fef2f2; color: #dc2626; border: 1px solid #fecaca;">Overdue</span>
                @endif
            </div>
                        ...
                        <div @click="open = !open" class="md:hidden" style="padding: 12px 14px; border-bottom: 1px solid #f1f5f9; cursor: pointer; {{ $task->status === 'done' ? 'opacity: 0.5;' : '' }}">
                            ...
                            <div style="padding-left: 22px; display: flex; align-items: center; gap: 8px;">
                                ...
                                <div style="position: relative;" @click.stop>
                                    ...
                                    <div x-show="tbcbOpen" x-cloak @click.outside="tbcbOpen = false"
                                         style="position: absolute; left: 0; bottom: 28px; z-index: 50; background: #fff; border: 1px solid #e5e7eb; border-radius: 10px; box-shadow: 0 8px 24px rgba(0,0,0,0.12); padding: 10px; min-width: 170px;">
                                        <p style="font-size: 8px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px; margin: 0 0 5px 2px;">Complete by</p>
                                        <input type="date" x-model="tbcbDate" style="width: 100%; font-size: 12px; border: 1px solid #e5e7eb; border-radius: 6px; padding: 5px 8px; color: #1e293b; outline: none;"
                                               @change="fetch('{{ url('admin/api/tasks') }}/{{ $task->id }}', { method:'PATCH', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'}, body: JSON.stringify({due_date: $event.target.value || null}) }).then(r => { if(r.ok) tbcbOpen = false; });">
                                        <button x-show="tbcbDate" @click="tbcbDate = ''; fetch('{{ url('admin/api/tasks') }}/{{ $task->id }}', { method:'PATCH', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'}, body: JSON.stringify({due_date: null}) }).then(r => { if(r.ok) tbcbOpen = false; });"
    ...
    {{-- Floating tasks (no plan assigned) --}}
    @if(isset($grouped['no-plan']) && $grouped['no-plan']->count() > 0)
        <div style="margin-bottom: 24px;">
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px; padding: 0 2px;">
                <span style="font-size: 15px; font-weight: 700; color: #1e293b;">📥 Unplanned</span>
                <span style="padding: 2px 8px; border-radius: 6px; font-size: 10px; font-weight: 600; background: #7c3aed18; color: #7c3aed; border: 1px solid #7c3aed30;">Floating</span>
                <span style="font-size: 11px; color: #94a3b8;">{{ $grouped['no-plan']->count() }} tasks</span>
            </div>
            <div class="admin-card" style="overflow: visible; border-radius: 12px;">
                @foreach($grouped['no-plan'] as $task)
                    @php $sc = $task->statusConfig; @endphp
                    <div x-data="{
                        open: false,
                        status: '{{ $task->status }}',
                        ...
                        editing: false,
                        title: '{{ addslashes($task->title) }}',
                        tbcbDate: '{{ $task->tbcb_date ? \Carbon\Carbon::parse($task->tbcb_date)->format('Y-m-d') : '' }}',
                        tbcbOpen: false,
                        statusLabels: { backlog:'Backlog', wip:'WIP', done:'Done', deferred:'Deferred' },
                        statusColors: { backlog:'#64748b', wip:'#f59e0b', done:'#22c55e', deferred:'#94a3b8' },
                    }">
                        ...
                        <div class="hidden md:grid" style="grid-template-columns: 36px 60px 1fr auto auto auto 32px; align-items: center; gap: 8px; padding: 10px 14px; border-bottom: 1px solid #f1f5f9; {{ $task->status === 'done' ? 'opacity: 0.5;' : '' }}">
                            ...
                            {{-- Col 3: Title --}}
                            <div style="min-width: 0;">
                                <span style="font-size: 13px; font-weight: 500; color: #1e293b; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: block;" x-text="title"></span>
                            </div>
                            {{-- Col 4: Pillar --}}
                            <span style="padding: 2px 8px; border-radius: 6px; font-size: 10px; font-weight: 600; background: #64748b18; color: #64748b;">{{ $task->pillar ? ucfirst($task->pillar) : '' }}</span>
                            {{-- Col 5: Rollover --}}
                            <div style="display: flex; align-items: center; gap: 4px; flex-shrink: 0;">
                                @if($task->is_rolled_over)
                                    <span style="padding: 1px 5px; border-radius: 4px; font-size: 9px; font-weight: 700; background: #fef3c7; color: #92400e; white-space: nowrap;">↩{{ $task->rollover_count }}</span>
                            ...
                            {{-- Col 6: TBCB date picker --}}
                            <div style="position: relative; flex-shrink: 0;">
                                <button @click="tbcbOpen = !tbcbOpen"
                                        style="padding: 2px 6px; border-radius: 5px; font-size: 9px; font-weight: 700; white-space: nowrap; border: 1px solid {{ $task->tbcb_date ? '#fde68a' : '#e5e7eb' }}; cursor: pointer; display: inline-flex; align-items: center; gap: 2px; transition: all 0.15s;
                                        {{ $task->tbcb_date ? 'background: #fffbeb; color: #d97706;' : 'background: #f8fafc; color: #94a3b8;' }}">
                                    <svg style="width: 9px; height: 9px;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                    <span x-text="tbcbDate ? new Date(tbcbDate + 'T00:00:00').toLocaleDateString('en-IN', {day:'numeric', month:'short'}) : 'TBCB'"></span>
                                </button>
                                <div x-show="tbcbOpen" x-cloak @click.outside="tbcbOpen = false"
                                    ...
                                    <p style="font-size: 8px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px; margin: 0 0 5px 2px;">Complete by</p>
                                    <input type="date" x-model="tbcbDate" style="width: 100%; font-size: 12px; border: 1px solid #e5e7eb; border-radius: 6px; padding: 5px 8px; color: #1e293b; outline: none;"
                                           @change="fetch('{{ url('admin/api/tasks') }}/{{ $task->id }}', { method:'PATCH', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'}, body: JSON.stringify({tbcb_date: $event.target.value || null}) }).then(r => { if(r.ok) tbcbOpen = false; });">
                                    <button x-show="tbcbDate" @click="tbcbDate = ''; fetch('{{ url('admin/api/tasks') }}/{{ $task->id }}', { method:'PATCH', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'}, body: JSON.stringify({tbcb_date: null}) }).then(r => { if(r.ok) tbcbOpen = false; });"
                                            style="width: 100%; margin-top: 5px; padding: 3px; border-radius: 5px; border: 1px solid #fecaca; background: #fef2f2; color: #dc2626; font-size: 9px; font-weight: 600; cursor: pointer;">Clear</button>
                                </div>
                            </div>
                            {{-- Col 7: Action toggle --}}
                        ...
                        {{-- ── MOBILE ROW ── --}}
                        <div class="md:hidden" style="padding: 10px 12px; border-bottom: 1px solid #f1f5f9; {{ $task->status === 'done' ? 'opacity: 0.5;' : '' }}">
                            <div @click="open = !open" style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                                <span style="width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; background: {{ $task->priority === 'must' ? '#ef4444' : ($task->priority === 'bonus' ? '#22c55e' : '#f59e0b') }};"></span>
                                <span style="flex: 1; font-size: 13px; font-weight: 500; color: #1e293b; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" x-text="title"></span>
                                <span x-show="vs > 0" style="font-size: 10px; font-weight: 700; font-family: ui-monospace, monospace;" :style="'color:' + vsBadgeColor(vs)" x-text="vs"></span>
                            </div>
                            <div style="display: flex; gap: 6px; margin-top: 4px; padding-left: 15px; flex-wrap: wrap;">
                                <span style="font-size: 9px; font-weight: 700; text-transform: uppercase;" :style="'color:' + statusColors[status]" x-text="statusLabels[status]"></span>
                                @if($task->pillar)
                                    <span style="font-size: 9px; color: #94a3b8;">{{ ucfirst($task->pillar) }}</span>
                                @endif
                                @if($task->tbcb_date)
                                    <span style="font-size: 9px; color: #d97706;">TBCB {{ \Carbon\Carbon::parse($task->tbcb_date)->format('j M') }}</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div x-show="open" x-cloak x-transition
                            ...
                            <div class="mb-3">
                                <p class="text-sm font-bold text-slate-900 mb-1" x-text="title"></p>
                                <p class="text-[11px] text-slate-500 leading-relaxed">
                                    Impact <strong style="color: {{ $task->impact_color ?? '#d19900' }}">{{ $task->impact_label ?? 'Medium' }}</strong>
                                    · VS <strong x-text="vs" :style="'color:' + vsBadgeColor(vs)"></strong>
                                    · {{ $task->pillar ? ucfirst($task->pillar) : 'Unassigned' }}
                                    @if($task->tbcb_date) · <span class="text-amber-600">TBCB {{ \Carbon\Carbon::parse($task->tbcb_date)->format('j M') }}</span> @endif
                                    @if($task->is_rolled_over) · <span class="text-amber-600">Rolled over {{ $task->rollover_count }}×</span> @endif
                                </p>
                            </div>
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                {{-- TBCB --}}
                                <div class="flex items-center gap-2.5">
                                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider w-14 shrink-0">TBCB</span>
                                    <div class="flex items-center gap-2">
                                        <input type="date" x-model="tbcbDate" class="text-xs border border-slate-200 rounded-lg px-2 py-1.5 text-slate-800 outline-none"
                                               @change="fetch('{{ url('admin/api/tasks') }}/{{ $task->id }}', { method:'PATCH', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'}, body: JSON.stringify({tbcb_date: $event.target.value || null}) });">
                                        <button x-show="tbcbDate" @click="tbcbDate = ''; fetch('{{ url('admin/api/tasks') }}/{{ $task->id }}', { method:'PATCH', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'}, body: JSON.stringify({tbcb_date: null}) });"
                                                class="px-2 py-1 rounded text-[9px] font-semibold cursor-pointer border border-red-200 bg-red-50 text-red-600">Clear</button>
                                    </div>
                                </div>
